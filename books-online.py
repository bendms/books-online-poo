...
description_livre = str(description_livre_clean_tag).replace("</p>", "")
...
# infos_livre = [product_page_url, universal_product_code, title, price_including_tax, price_excluding_tax, product_description, category, review_rating, image_url]
liste_livre = [title_livre]
...
with open('livre_info.csv', 'w', encoding='UTF-8') as f:
    writer = csv.writer(f)
    liste_csv.append(liste_entetes)
    liste_csv.append(liste_livre)
